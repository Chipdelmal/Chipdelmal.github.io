...
With the clustering done, I wanted to try out creating a simple application to compare builds one against the other. To do this, I coded up a [Dash](https://dash.plotly.com/) front-end with a [plotly](https://plotly.com/python/) radar plot with each build stats, and a table of builds that are similar to the main selected build.
...
Now, the [hiplot](https://facebookresearch.github.io/hiplot/) framework is quirky and a bit obscure in its documentation so I couldn't get any custom color palette to work but here are some snapshots of the [exported HTML](https://github.com/Chipdelmal/MKW_ParallelPlot/blob/main/plots/parallel.html), which can be readily [downloaded](https://github.com/Chipdelmal/MKW_ParallelPlot/blob/main/plots/parallel.html) and run in any browser:
