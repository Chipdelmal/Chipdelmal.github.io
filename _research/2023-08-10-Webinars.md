...
As part of our work with the Melinda & Gates Foundation we've developed a coupkle of webinars for our software packages using [Docker](https://www.docker.com/) with other packaging tools to make it as easy as possible for our audience to get the materials up and running.
...
In the case of the [R](https://www.r-project.org/) webinar, we decided to use the [rocker/rstudio](https://hub.docker.com/r/rocker/rstudio) image as it provided users with an experience that would be almost the same as the one they'd have the most familiarity with. This image comes with RStudio pre-installed and runs it on our web browser, so in this case we just needed to copy our materials into the image, and to install MGDrivE from CRAN to get our materials up and running.
